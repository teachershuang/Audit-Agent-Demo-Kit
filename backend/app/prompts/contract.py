...
def build_section_candidate_prompt(page_payload: list[dict[str, Any]]) -> PromptBundle:
    return PromptBundle(
        system=(
            ...
            "严格排除：合同标题、合同编号、填写说明、页眉页脚、签章说明、登记法人、登记材料、合同类别、合同交易额、技术交易额、甲方/乙方字段名。\n"
            "只有明确带有章节/条款编号的锚点才可输出，例如“第一条”“第二条”“一、”“（一）”“1.1”。\n"
            "不要把“第十六条：本合同一式肆份，具有同等法律效力。”整句作为标题，只输出“第十六条”或“第十六条 合同份数/效力”。\n"
            "输出必须是中文 JSON。"
        ),
        user=(
            f"合同分页与文本块如下：\n{_dump(page_payload)}\n\n"
            ...
            "\n过滤规则：没有章节编号的候选不要输出；字段名、合同首页标题、填写说明页内容不要输出为章节。"
        ),
    )
...
def build_section_merge_prompt(
    ...
    return PromptBundle(
        system=(
            ...
            "最终章节必须是合同真实条款结构。排除合同标题、填写说明、合同编号、登记法人、合同金额、技术交易额等字段项。\n"
            "同一编号重复出现时只保留第一次作为章节锚点，后续正文归入该章节摘要。\n"
            "标题应短且规范：优先使用“第X条 + 业务主题”，不要输出整句正文。\n"
            "输出必须是中文 JSON。"
        ),
        user=(
